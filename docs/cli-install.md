...
| PowerShell: `ftc.ps1 cannot be loaded` / execution policy | Use **`ftc.cmd doctor`** (and other `ftc.cmd …` commands), run from **Command Prompt**, or set `Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Scope CurrentUser` once. npm installs global CLIs as both `.ps1` and `.cmd` on Windows.              |
